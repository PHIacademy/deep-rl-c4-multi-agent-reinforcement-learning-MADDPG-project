...
- **Warmup phase.** 300 episodes of pure random action selection were used to pre-fill the shared replay buffer before any learning began, to avoid the critic locking onto a trivial "predict ~0 Q everywhere" solution from a buffer that otherwise starts out almost entirely uniform, zero-reward transitions.
